...
(function($) {
    $(document).ready(function() {
      ...
      var prefix = '#id_goodsreceiptpoitem_set-';
      var pricePrefix = '#id_goodsreceiptpoitem_set-{0}-price';
      var quantityPrefix = '#id_goodsreceiptpoitem_set-{0}-quantity';
      var priceTotalPrefix = '#id_goodsreceiptpoitem_set-{0}-priceTotal';
      ...
      function calculateTotalAmount() {
        var total = 0;
        $('input[name^="goodsreceiptpoitem_set-"][name$="-priceTotal"]').each(function() {
...
function calculateTotalQty() {
  var total = 0;
  $('input[name^="goodsreceiptpoitem_set-"][name$="-quantity"]').each(function() {
  ...
  return total.toFixed(4);
      ...
      function handleFieldChanges() {
        $('input[name^="goodsreceiptpoitem_set-"][name$="-price"], input[name^="goodsreceiptpoitem_set-"][name$="-quantity"]').on('change', function() {
...
(function($) {
  $(document).ready(function() {
      ...
      $('input[name^="goodsreceiptpoitem_set-"][name$="-code"]').each(function() {
...
(function($) {
  $(document).ready(function() {
          ...
          $("#id_purchaseOrder").on('change', function() {
            const purchaseOrder = $(this).val();
            const inputElement = $(this);
      

            $.ajax({
              type: 'POST',
              url: '/Purchasing/purchaseorderinfo/',
              data: {
...
(function($) {
  $(document).ready(function() {
      ...
      $('input[name^="goodsreceiptpoitem_set-"][name$="-lineNo"]').each(function() {
          $(this).on('change', function() {

            
            const inputValue = parseInt($(this).val(), 10);
            const receiptNo = $(this).closest('tr').find('.field-purchareOrderNo input').val();
