...
#ডেট অনুযায়ী আইটেম Stock রিপোর্ট 
def item_stock_between_date(request):
    form = DateFilterForm(request.POST)
    ...
    if request.method == 'POST' and form.is_valid():
        ...
        if start_date and end_date:
            try:
                items_within_range = Stock.objects.filter(created__range=(start_date, end_date))
                
                return render(request, 'inventory/item_stock_between_date.html', {'items': items_within_range})
            except ValueError:
                # Handle invalid date format
                pass
    
    return render(request, 'inventory/item_stock_between_date.html', {'form': form})




#ডেট অনুযায়ী আইটেম Stock রিপোর্ট 
def issue_for_production_between_date(request):
    form = DateFilterForm(request.POST)
    ...
    if request.method == 'POST' and form.is_valid():
        ...
        if start_date and end_date:
            try:
                items_within_range = IssueForProductionItem.objects.filter(created__range=(start_date, end_date))
                
                return render(request, 'inventory/issue_for_production_between_date.html', {'items': items_within_range})
            except ValueError:
                # Handle invalid date format
                pass
    
    return render(request, 'inventory/issue_for_production_between_date.html', {'form': form})
...
#ডেট অনুযায়ী issue for production রিপোর্ট ডিপার্টমেন্ট সিলেক্ট করে 
def issue_for_production_between_date_by_department(request):
    form = DateDepartmentFilter(request.POST)
    if request.method == 'POST' and form.is_valid():
            start_date = form.cleaned_data['start_date']
            end_date = form.cleaned_data['end_date']
            ...
            # Filter data based on form input
            items = IssueForProductionItem.objects.filter(
                created__range=(start_date, end_date),
                department=department.name
            )    
    
            return render(request, 'inventory/issue_for_production_between_date_by_department.html', {'items': items})
    else:
        form = DateDepartmentFilter()

    return render(request, 'inventory/issue_for_production_between_date_by_department.html', {'form': form})
...
#ডেট অনুযায়ী ডিপার্টমেন্ট issue for production টোটাল সামারি  রিপোর্ট
def issue_for_production_department_summary_by_dates(request):
    form = DateFilterForm(request.POST)
    ...
    if request.method == 'POST' and form.is_valid():
        ...
        if start_date and end_date:
            try:
                # Retrieve total quantity grouped by department based on the given order number and dates
                quantity_by_department = IssueForProductionItem.objects.filter(
                   created__range=(start_date, end_date)
                ).values('department').annotate(total_quantity=Sum('quantity'))
                
                return render(request, 'inventory/issue_for_production_department_summary_by_dates.html', {'quantity_by_department': quantity_by_department})
            except ValueError:
                # Handle invalid date format
                pass
    
    return render(request, 'inventory/issue_for_production_department_summary_by_dates.html', {'form': form})
...
# ১ বছরের মাস অনুযায়ী রিপোর্ট issue for production
def issue_for_production_monthly_data(request):
    year = None
    total_quantity = 0
    monthly_totals_dict = {i: 0 for i in range(1, 13)}
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            year = form.cleaned_data['year']
            # Calculate the 12-month total quantity for the selected year
            total_quantity = IssueForProductionItem.objects.filter(
                created__year=year
            ).aggregate(total_qty=Sum('quantity'))['total_qty'] or 0

            # Calculate monthly totals for the selected year
            monthly_totals = IssueForProductionItem.objects.filter(
                created__year=year
            ).values('created__month').annotate(monthly_qty=Sum('quantity'))
    ...
    return render(request, 'inventory/issue_for_production_monthly_data.html', {
        'year': year,
        'total_quantity': total_quantity,
        'monthly_totals_dict': monthly_totals_dict,
        'form': form,
